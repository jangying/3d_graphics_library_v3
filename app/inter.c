...
{                                           /* application, the model */
 struct M_polygon_object* p;                /* and light sources */
 struct L_light** l;
 int no_l;
...
void frame(void)                            /* called to display a frame */
{
 ...
 M_render_polygon_object(app_data->p,0,0,0,alp,bet,gam);

 G_text(0,0,"<enter> <arrows,x,s> <p,w,f,g,t>",255,255,255);
...
{
 app_data=D_data("data/intr_app.dat");

 HW_init_screen("3Dgpl3 - Interior scene",300,300);

 M_init_rendering(M_TEXTURED);              /* rendering option */
 M_init_polygon_object(app_data->p);
 L_init_lights(app_data->l,app_data->no_l);
 M_light_polygon_object(app_data->p,0,0,0,0,0,0);
