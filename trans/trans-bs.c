/** 3Dgpl3 ************************************************\
 * 3D transformations of coordinates.                     *
 *                                                        *
 * Ifdefs:                                                *
 ...
 *  T_concatinate_self_world The two above matrices;      *
 *  T_concatinated_rotation  From object into view space; *
 *                                                        *
...
#include "../hardware/hardware.h"           /* HW_screen_x_size etc. */
...
#if defined(_FIXED_)
HW_fixed T_wx1,T_wx2,T_wx3,T_wy1,T_wy2,T_wy3,T_wz1,T_wz2,T_wz3;
HW_fixed T_sx1,T_sx2,T_sx3,T_sy1,T_sy2,T_sy3,T_sz1,T_sz2,T_sz3;
HW_fixed T_cx1,T_cx2,T_cx3,T_cy1,T_cy2,T_cy3,T_cz1,T_cz2,T_cz3;
HW_fixed T_tx,T_ty,T_tz;
HW_fixed T_sin[256],T_cos[256];             /* precalculated */
#endif
#if defined(_FLOAT_)
 ...
 * assumed to be in pseudo-degrees in the range [0,255].  *
 *                                                        *
...
{
#if defined(_FIXED_)
 HW_fixed cosalp,sinalp,cosbet,sinbet,cosgam,singam;
#endif
#if defined(_FLOAT_)
 ...
 * in pseudo-degrees in the range [0,255].                *
 *                                                        *
...
/**********************************************************\
 * Concatinating matrices of object to world rotation,    *
...
void T_perspective(int *from,int *to,int dimension,
                   int length,int log_focus)
{
 ...
 {                                          /* NB! Z is not changed */
  *to++=(int)((((long)from[0])<<log_focus)/from[2])+HW_screen_x_size/2;
  *to++=(int)((((long)from[1])<<log_focus)/from[2])+HW_screen_y_size/2;
                                            /* also translates to screen */
#if defined(_Z_BUFFER_)                     /* centre */
  *to++=T_ONE_OVER_Z_CONST/(long)from[2];   /* 1/Z is left in the tuple */
#endif

  memcpy(to,from+=3,rem_dim*sizeof(int));   /* remaining values are passed */
 }
}
...
{
#if defined(_FIXED_)
 HW_fixed a[T_MAX_MATRIX_SIZE][T_MAX_MATRIX_SIZE];
 HW_fixed b[T_MAX_MATRIX_SIZE][T_MAX_MATRIX_SIZE];
 HW_fixed x[T_MAX_MATRIX_SIZE][T_MAX_MATRIX_SIZE];
 HW_fixed max,tmp,pivot,sum;
#endif
#if defined(_FLOAT_)
 ...
 {
#if defined(_FIXED_)
  for(j=0;j<n;j++) a[i][j]=(HW_fixed)(ia[i][j]>>log_source_multiplyer);
  for(j=0;j<m;j++) b[i][j]=(HW_fixed)ib[i][j];
#endif
#if defined(_FLOAT_)
